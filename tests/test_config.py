...
class TestDetectSubscription:
    def test_returns_info_on_success(self, monkeypatch):
        ...
        monkeypatch.setattr("azure_jobs.core.config.az_cli.subprocess.run", mock_run)
    ...
    def test_returns_none_when_az_missing(self, monkeypatch):
        monkeypatch.setattr(
            "azure_jobs.core.config.az_cli.subprocess.run",
...
class TestPickWorkspace:
    def test_selects_by_number(self, monkeypatch):
        ...
        monkeypatch.setattr("azure_jobs.core.config.prompts._prompt", lambda *a, **kw: 2)
    ...
    def test_zero_returns_none_for_manual(self, monkeypatch):
        ...
        monkeypatch.setattr("azure_jobs.core.config.prompts._prompt", lambda *a, **kw: 0)
...
class TestGetWorkspaceConfig:
    ...
    def test_full_auto_detect_flow(self, aj_config, monkeypatch):
        ...
        monkeypatch.setattr("azure_jobs.core.config.az_cli.subprocess.run", mock_run)
        monkeypatch.setattr("azure_jobs.core.config.prompts._prompt", lambda *a, **kw: 1)  # pick workspace #1
        result = get_workspace_config()
    ...
    def test_manual_fallback_when_no_workspaces_found(self, aj_config, monkeypatch):
        ...
        monkeypatch.setattr("azure_jobs.core.config.prompts._prompt", lambda *a, **kw: next(inputs))
        result = get_workspace_config()
