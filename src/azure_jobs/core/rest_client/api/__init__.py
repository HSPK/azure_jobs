...
Each API class wraps a single domain area (jobs, resources, blob)
and is composed into :class:`azure_jobs.core.rest_client.AzureMLClient`.

Only the public façade is re-exported. Helpers like ``extract_rest_job``,
``extract_error_message`` and ``trim_arm_id`` remain internal to
:mod:`.extract`; ``RunHistoryAPI`` is an internal collaborator of
:class:`.JobsAPI` and is reachable via ``client.jobs.get_run_log_urls`` etc.
"""
