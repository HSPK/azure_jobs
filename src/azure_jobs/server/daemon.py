...
from azure_jobs.shared.contract.rpc import FrameWriter, notification, serve_connection
...
class Connection:
    ...
    def push(self, note: Notification) -> None:
        self.push_raw("notify", note.to_json())

    def push_raw(self, method: str, params: Mapping[str, Any]) -> None:
        try:
            self.writer.send(notification(method, dict(params)))
        except Exception:
            log.debug("Dropping a %s push for a closed connection", method)
...
class Daemon:
    """Socket server exposing :mod:`azure_jobs.shared.contract` to local clients."""
...
def _m_submit_run(daemon: Daemon, params: Mapping[str, Any], conn: Connection) -> Any:
    """Run a submission, streaming progress back on this connection.

    Progress is a callback, which cannot cross a socket; pushing it as
    correlated notifications keeps `aj run` as informative as it was before
    submission moved into the daemon.
    """
    backend = _jobs(daemon, params)
    stream_id = str(params.get("stream") or "")
    ...
    outcome = backend.submitter.submit(
